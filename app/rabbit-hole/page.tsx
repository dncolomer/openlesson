      ...
      <div className="relative z-10 mx-auto flex h-[100svh] w-full max-w-7xl flex-col px-5 py-5 sm:px-8 lg:px-10">
        <header className="pointer-events-none fixed left-5 right-5 top-5 z-20 flex items-center justify-center gap-4 sm:left-8 sm:right-8 lg:static lg:pointer-events-auto lg:justify-between">
          ...
          <div className="pointer-events-auto flex h-8 overflow-hidden rounded-full bg-black text-xs font-semibold leading-none ring-1 ring-white/10">
            ...
            <div className="flex w-[72px] items-center justify-center text-white/80 sm:w-[88px]">{status ? `${status.playsAvailable} play${status.playsAvailable === 1 ? "" : "s"}` : "Loading"}</div>
            {status && <div className="flex w-[72px] items-center justify-center border-l border-white/10 text-white/80 sm:w-[88px]">{status.points} pts</div>}
            {status && <div className="flex w-[72px] items-center justify-center border-l border-white/10 text-white/80 sm:w-[88px]">{status.globalRank ? `#${status.globalRank}` : "--"}</div>}
            <button onClick={unlockPlays} className="flex w-[92px] items-center justify-center bg-white text-black transition hover:bg-[#f2ead7] active:scale-[0.98] sm:w-[108px]">+3 for $1.99</button>
          </div>
        </header>
