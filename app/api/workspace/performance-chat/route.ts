...
export async function POST(req: NextRequest) {
  try {
    ...
    const { workspaceId, message, conversationHistory = [], fileIds = [] } = body;
    ...
    if (message.trim() === "/debug") {
      ...
      const debug = await fetchPerformanceDebug(debugClient, workspaceId, user.id, isOwner, profile?.organization_id, isOrgAdmin);
    ...
    if (activeFileIds.length === 0) {
      ...
      const fileName = `performance-data-${workspaceId}-${Date.now()}.json`;
...
async function fetchPerformanceDebug(
  ...
  const { data: pnsRows, error: pnsError } = await supabase
    .from("block_sessions")
    .select("session_id, user_id, block_id")
  ...
  return {
    ...
    plan,
    counts: {
      block_sessions: pnsRows?.length || 0,
      blocks: nodeRows?.length || 0,
      blocks_with_session_id: directSessionIds.length,
    ...
    errors: {
      block_sessions: pnsError?.message || null,
      blocks: nodeError?.message || null,
...
function formatPerformanceDebug(debug: Awaited<ReturnType<typeof fetchPerformanceDebug>>): string {
  return `Performance debug for plan \`${debug.workspace_id}\`:
...
- block_sessions: ${debug.counts.block_sessions}
- blocks: ${debug.counts.blocks}
- blocks_with_session_id: ${debug.counts.blocks_with_session_id}
...
async function fetchPerformanceData(
  ...
  return fetchLinkedPlanPerformanceData(
    supabase, workspaceId, workspaceTitle, requestingUserId, canSeeAllUsers, isOwner, organizationId, isOrgAdmin
  );
}

/**
 * Fetch performance data from every supported plan-session link path. Plans can
 * be linked through block_sessions, blocks.session_id, or session
...
async function fetchLinkedPlanPerformanceData(
  ...
  if (canSeeAllUsers) {
    ...
    for (const link of links || []) {
      sessionNodeMap.set(link.session_id, link.block_id);
    }
    ...
    for (const session of metadataSessions || []) {
      ...
      const blockId = typeof metadata?.block_id === "string" ? metadata.block_id : null;
      if (blockId || !sessionNodeMap.has(session.id)) {
        sessionNodeMap.set(session.id, blockId);
      }
    }

    const sessionIds = Array.from(sessionNodeMap.keys());
    const blockIds = Array.from(new Set(Array.from(sessionNodeMap.values()).filter(Boolean))) as string[];
    ...
    const { data: nodes } = blockIds.length > 0
      ? await supabase
        .from("blocks")
        .select("id, title")
        .in("id", blockIds)
    ...
    const blockTitleMap = new Map((nodes || []).map(node => [node.id, node.title]));
    ...
    rows = (sessions || []).flatMap(session => {
      ...
      const blockId = sessionNodeMap.get(session.id) || null;
      return [{
        ...
        ended_at: null,
        block_id: blockId,
        block_title: blockId ? blockTitleMap.get(blockId) || null : null,
      }];
    });
  } else {
    ...
    rows = (sessions || []).map(s => ({
      ...
      ended_at: null,
      block_id: sessionNodeMap.get(s.id),
      block_title: nodeMap.get(sessionNodeMap.get(s.id) || "") || null,
    }));
  }
