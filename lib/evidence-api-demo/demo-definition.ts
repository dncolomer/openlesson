...
export interface EvidenceApiDemoDefinition {
  id: string;
  productName: string;
  ...
  /** Stored on learning_plans.description — must read like a real customer program */
...
export function getDemoWorkspaceModelFile(demo: EvidenceApiDemoDefinition): {
  name: string;
  mime_type: string;
  data: string;
} {
  ...
  return [
    {
      id: "wait_1_day",
      label: "Wait 1 day",
      description: "Record a 24h idle gap between sessions (uploads time-gap evidence).",
...
This document defines how partner integrations should collect evidence and how OpenLesson evaluates
competency. Attach at workspace creation via \`POST /api/v2/agent/workspaces\` with \`files[]\` so
performance analysis, evidence-spec generation, and integration-skill regeneration stay grounded in
