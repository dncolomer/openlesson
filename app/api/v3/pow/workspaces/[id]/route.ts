...
export async function GET(req: NextRequest, { params }: RouteProps) {
  ...
  const { data: workspace } = await supabase
    .from("workspaces")
    .select("id, title, root_topic, description, notes, workspace_goal, status, created_at, updated_at, user_id, organization_id, guest_user_id")
    .eq("id", workspaceId)
    .single();
...
export async function PATCH(req: NextRequest, { params }: RouteProps) {
  ...
  const { data: workspace } = await supabase
    .from("workspaces")
    .select("id, user_id, organization_id, guest_user_id, workspace_goal")
    .eq("id", workspaceId)
    .single();
  ...
  if (!("workspace_goal" in body)) {
    return errorResponse(400, "validation_error", "workspace_goal is required");
  }

  const workspaceGoal =
    body.workspace_goal === null ? null : normalizeWorkspaceGoal(body.workspace_goal);

  if (body.workspace_goal !== null && !workspaceGoal) {
    return errorResponse(400, "validation_error", "workspace_goal must be a non-empty string or null");
  }

  const { data: updated, error } = await supabase
    .from("workspaces")
    .update({ workspace_goal: workspaceGoal })
    .eq("id", workspaceId)
    .select("id, workspace_goal, updated_at")
    .single();
  ...
  return NextResponse.json({
    ...
    workspace_goal: updated.workspace_goal,
