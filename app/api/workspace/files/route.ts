...
export async function GET(req: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: "workspaceId or fileId required" }, { status: 400 });
  } catch (err) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!workspaceId || !fileName || !mimeType || !base64Data) {
      return NextResponse.json({ error: "workspaceId, fileName, mimeType, and data are required" }, { status: 400 });
    }
