...
/** @deprecated Use extractGameCoaching for structured direction/event coaching. */
export function extractGameTips(report: PerformanceReport | null): string[] {
  const coaching = extractGameCoaching(report);
  return [...coaching.events, ...coaching.directions, ...coaching.gapRepairs];
}