...
export async function POST(req: NextRequest) {
  try {
    ...
    const request = parseIntegrationSkillRequest({
      ...
      prefetch_evidence_spec: body.prefetch_evidence_spec,
    });
    ...
    if (request.prefetch_evidence_spec) {
      ...
      if (evidenceSchemaRequest) {
        try {
          ...
          console.error("[learning-plan/integration-skill] Evidence spec prefetch failed:", error);
        }
      }
    }
    ...
    console.error("[learning-plan/integration-skill] Unhandled error:", error);
    return NextResponse.json(
      {
