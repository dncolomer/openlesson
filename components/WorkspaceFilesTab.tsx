...
export function WorkspaceFilesTab({ workspaceId, isOwner }: WorkspaceFilesTabProps) {
  const { t } = useI18n();
    ...
    <div className="h-full overflow-y-auto">
      <div className="w-full space-y-4 p-1">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h3 className="text-sm font-medium text-white">{t('workspaceFiles.title')}</h3>
            <p className="text-xs text-neutral-500 mt-0.5">{t('workspaceFiles.subtitle')}</p>
          </div>
        ...
        {files.length > 0 ? (
          ...
          !isOwner && (
            <div className="flex flex-col items-center gap-3 py-16 text-neutral-600">
              ...
              <span className="text-sm">{t('workspaceFiles.noFiles')}</span>
            </div>
          )
        )}
        ...
        {isOwner && files.length === 0 && !atLimit && (
          <p className="text-xs text-neutral-500 -mt-2">{t('workspaceFiles.noFiles')}</p>
        )}
      </div>
    </div>
  );
