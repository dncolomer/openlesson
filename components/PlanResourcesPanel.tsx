...
export function PlanResourcesPanel({ planId }: PlanResourcesPanelProps) {
  const { t } = useI18n();
  ...
  useEffect(() => {
    fetch(`/api/learning-plan/files?planId=${planId}`)
      .then(r => r.json())
      .then(data => {
  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <div className="px-3 py-2 border-b border-neutral-800/60">
        ...
        <p className="text-[10px] text-neutral-600 mt-0.5">{t('planFiles.resourcesDesc')}</p>
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-2">
        {loading ? (
          ...
          <div className="flex flex-col items-center gap-2 py-12 text-neutral-600">
            ...
            <span className="text-xs text-center">{t('planFiles.noResourcesInSession')}</span>
          </div>
        ) : (
