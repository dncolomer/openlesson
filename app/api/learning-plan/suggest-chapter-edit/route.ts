...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data: session } = await supabase
      .from("sessions")
      .select("id, user_id, plan_id, problem")
      .eq("id", sessionId)
      .single();
    ...
    if (session.plan_id) {
      try {
        const ctx = await buildWorkspacePerformanceContext({
          ...
          workspaceId: session.plan_id,
        });
        performanceNote = `Evidence: ${ctx.payload.counts.evidence_artifacts} artifacts, ${ctx.payload.counts.tap_sessions} TAP sessions, ${ctx.payload.counts.linked_sessions} linked sessions. Blocks: ${ctx.payload.blocks.map((b) => b.title).filter(Boolean).join(", ") || "none"}`;
      } catch {
        // optional context
      }
    }
