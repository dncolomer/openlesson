    ...
    return NextResponse.json(
      await withProofOfWorkApiResponse(
        ...
        },
        {
          ...
          artifact_summary:
            scored.report.summary || `Augmentation score ${scored.report.score}`,
        }
      )
    );
  } catch (error) {
    console.error("[agent/augmentation-score] failed:", error);
    const message = error instanceof Error ? error.message : "Failed to generate augmentation score";
