...
export function buildTapbenchSkillsMarkdown(input: TapbenchSkillsMdInput): string {
  ...
  const lines: string[] = [
    ...
    `When the token expires, the API returns **401** with \`session_expired\` on \`error.code\` plus \`error.details.expires_at\`, \`error.details.remaining_ms\`, and \`error.details.tapbench\` — **stop sending** proof of work.`,
    ``,
    `## Exercise`,
    ``,
  ];
