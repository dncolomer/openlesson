...
export async function GET(req: NextRequest, { params }: RouteProps) {
  ...
  return NextResponse.json({
    ...
    stash: {
      ...
      note: "Pass session_token as X-Tapbench-Session header (or body.session_token) on stash/submit until remaining_ms is 0. Flushed PoW is TAP thought-trace shaped (metadata.text, tool_name stash_submit_api) and attributed to guest_user_id.",
    },
    url: `${base.replace(/\/$/, "")}/tapbench/${resolved.session_token}`,
  });
}
