...
describe("ILE map-first session chrome (shipped surface)", () => {
  it("is full-viewport map with overlay widgets, not a tools/Helios split", () => {
    ...
    expect(tools).toContain("h-14 w-full object-cover");
    expect(tools).not.toContain("aspect-video w-full object-cover");
    ...
    expect(chrome).toContain("museStatus === \"streaming\" ? <EegMiniPreview");
