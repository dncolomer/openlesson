...
export function buildTapbenchSkillsMarkdown(input: TapbenchSkillsMdInput): string {
  ...
  lines.push(
    ...
    ``,
    `1. **Buffer** thought/work units (prefer JSON with a clear \`text\` / \`reasoning\` field — same free text humans speak in TAP).`,
    `2. **Stash** (System 1) or **Submit** (System 2) to flush into durable PoW.`,
    `3. On flush, the server **aligns** your unit with human TAP thought traces:`,
    ...
    `4. Repeat until \`remaining_ms\` is 0 or requests return \`session_expired\`.`,
    ``,
    `| Step | Method | Path |`,
    `| --- | --- | --- |`,
    `| Buffer PoW | \`POST\` | \`${ingestPath}\` |`,
    `| Stash (System 1) | \`POST\` | \`${stashPath}\` |`,
    `| Submit (System 2) | \`POST\` | \`${submitPath}\` |`,
    ``,
    ...
    `- \`${abs(submitPath)}\``,
    ``,
    `### Example: buffer a spoken-style thought (TAP-comparable)`,
    ``,
    "```http",
    `POST ${abs(ingestPath)}`,
    `${TAPBENCH_SESSION_HEADER}: ${sessionToken || "<session_token>"}`,
    ...
    `  "data": "<base64 of {\\"text\\":\\"Lead indexes with tenant_id...\\"} >",`,
    `  "tool_name": "reason",`,
    `  "tool_action": "think_aloud",`,
    ...
    "```",
    ``,
    `Put the actual answer / reasoning in JSON fields such as \`text\`, \`reasoning\`, or \`content\` — these become \`metadata.text\` on flush (human TAP parity for region embeddings).`,
    ``,
    `### Example: stash then submit`,
    ``,
    "```http",
    `POST ${abs(stashPath)}`,
    `${TAPBENCH_SESSION_HEADER}: ${sessionToken || "<session_token>"}`,
