...
export async function POST(req: NextRequest) {
  try {
    ...
    const history = messages
      .slice(-12)
      .map((message: any) => `${message.role === "assistant" ? "GHL" : "Learner"}: ${String(message.content || "").slice(0, 2000)}`)
      .join("\n\n");
    ...
    if (!response.success || !response.data) {
      return NextResponse.json({ error: response.error || "Failed to generate GHL response" }, { status: 500 });
    }

    return NextResponse.json({ message: response.data });
  } catch (error) {
    console.error("[workspace-ghl-score/chat] Error:", error);
