...
export async function POST(request: NextRequest) {
  ...
  if (typeof playId !== "string" || !Number.isInteger(choiceIndex) || choiceIndex < 0 || choiceIndex > 2) return jsonError(400, "Invalid answer");
  ...
  if (!play) return jsonError(404, "Play not found");
  ...
  const interview = play.interview as { correctIndex?: number } | null;
  if (!Number.isInteger(interview?.correctIndex)) return jsonError(400, "Invalid interview");
